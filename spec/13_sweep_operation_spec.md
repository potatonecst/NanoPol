...
バックエンドはリクエスト受信後、以下を順番に実行する。

1. 入力値を検証する。
2. Sweep の方向を決定する。
3. 加速距離から助走距離を算出する。
4. `start` と `end` の両方を同じだけ平行移動し、助走区間を確保する。
5. どちらかが負の角度になった場合は、`360` 度を両方に加算して補正する。
6. 原点をまたいでも危険な負パルスにならないかを確認する。
7. 実行可能であれば `operation_id` を発行し、Sweep を開始する。

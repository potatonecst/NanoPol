...
*   **コンポーネント**:
    ...
    ※**長いパスのホバー確認**: パスが長くて表示欄からはみ出る場合でも、マウスホバーすることで全体の絶対パスがツールチップ表示（`title`属性）される仕様とします。
*   **挙動**:
...
*   **機能**:
    1.  **現在適用されている出力パス (Output Directory) の直接編集**:
        ...
        ※**長いパスのホバー確認**: パスが長く入力欄に収まらない場合でも、マウスホバーすることで全体の絶対パスがツールチップ表示（`title`属性）される仕様とします。

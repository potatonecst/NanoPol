...
サンプルの「実験ノート」としての役割を持つ。各測定セッションの**マクロな進行状況と開始・終了時刻**を記録する。ROIの細かい座標などは含めない。
...
```json
{
  ...
  "rois": [
    {
      "id": "roi_0",
      "initial": { "x": 500, "y": 400, "w": 33, "h": 33 }, // ユーザーが手動で置いた初期位置
      "final_aligned": { "x": 502, "y": 399, "w": 33, "h": 33 } // 重み付き平均で確定し、本番でロックされた位置
    }
  ]
}
```

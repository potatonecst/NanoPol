...
*   **解決策 (キャッシュパターン):**
    ...
    2.  `lifespan` 起動時に `asyncio.create_task(stage_monitor_loop())` をバックグラウンドタスクとして起動します。ステージ静止時は通信負荷・USB競合を抑えるために1.0秒（1000ms）間隔、ステージ移動中（`app_state.is_busy`）のみ0.1秒（100ms）間隔へと動的にポーリング頻度を切り替え、状態を尋ねて `app_state` を最新化し続けます。
    3.  **【即時イベント割り込みによる角度追従（`stage_wakeup_event`）】**: 静止時（1.0秒間隔）のスリープ中にステージ移動コマンド（`move_absolute`, `move_relative`, `home` 等）が発行された際、ただ `app_state.is_busy = True` にするだけでは監視タスクが1.0秒のタイマーで眠り続けてしまい、出だしの最大1秒間画面の角度更新がストップするタイムラグ（カクつき）が発生します。これを防ぐため、非同期イベント通知オブジェクト `stage_wakeup_event = asyncio.Event()` を導入しました。移動命令の発行と同時に `request_stage_wakeup()` を介してスリープ中の監視タスクを0秒で即座に叩き起こし（`asyncio.wait_for` の解除）、タイムラグゼロで最新角度の読み取りと0.1秒高速モードへの移行を達成しています。
    4.  フロントエンドが `/stage/position` を叩いた際は、シリアルデバイスには一切アクセスせず、この `app_state` の中身を即座に（0ミリ秒で）返します。

これにより、APIのレスポンス速度が劇的に向上し、ハードウェアへの負荷と角度表示の遅延がともに最小限に抑えられています。
...
4. **ハードウェアとソフトウェアの同期:**
    - `_run_sweep_operation` 内でステージに移動命令 (`move_absolute` 等) を送った直後、別タスクである `stage_monitor_loop` (移動中は0.1秒/100ms周期へ自動加速) が `is_busy=True` を検知するまで、明示的に待機します。これにより、「移動が開始される前に完了判定ループをすり抜けてしまう」というレースコンディションを防ぎ、遅延ゼロの正確な録画トリガーを実現しています。
...
### 2.9 映像配信エンドポイント (`/camera/video_feed`) とゾンビ接続の自動排除

カメラから取得したプレビュー映像をブラウザ向けに MJPEG (`multipart/x-mixed-replace`) 形式でストリーミング配信するエンドポイントです。

#### 接続リーク（ゾンビ接続）に伴うスレッド枯渇とフリーズの回避策
本エンドポイントは通常の同期関数（`def`）として定義されており、FastAPIのスレッドプール上で並行実行されます。WebView2等のブラウザ挙動やレイアウトのリサイズによって古いMJPEG接続が明示的に切断されず残留（ゾンビ化）した場合、スレッドプールが枯渇し、露出設定（`/camera/config`）などの他のすべてのREST APIリクエストを処理するためのスレッドが不足してフリーズする問題が起きます。

これを防ぐため、以下の**「最新接続ID照合による自動ゾンビ駆逐」**を実装しています。
1. `/camera/video_feed` への接続要求が来るたびに、一意な `uuid` (ストリームID) を新規発行して `camera.active_stream_id` に代入。
2. `CameraController.generate_frames` の映像配信ループの各周回において、自分自身のストリームIDが最新であるかを検証。
3. `my_stream_id != camera.active_stream_id` となった瞬間に、古い無限ループを即座に `break` してスレッドを安全に解放し、スレッドプールへ返却します。
...
| メソッド | パス | 役割 | 備考 |
| :--- | :--- | :--- | :--- |
| `GET` | `/measurement/sessions` | 測定セッション一覧取得 | `date_dir` 引数で過去日の参照が可能（省略時は今日）。各セッションのメタデータ（作成日時、測定進捗）も併せて取得します。 |
...
*   **集約されたセッションメタデータ**: `GET /measurement/sessions` のレスポンスに含まれる `sessions` は、単なる名前の配列（文字列）ではなく、各サンプルの `settings.json` から抽出された `created_at` (作成日時) および `progress` (完了した測定カテゴリ数/4。例: "2/4") を含むオブジェクトの配列に変更されています。これにより、フロントエンド側で一覧表示する際の通信回数やオーバーヘッドを削減しています。
...
*   **測定完了時の飽和警告（Quality Tracking）**: 自動測定シーケンスがエラーなく最後まで完了（`succeeded`）した場合であっても、各角度での撮影過程で一度でもカメラのビット深度の限界（白飛び）に達した「飽和（Saturation）」が含まれていた場合、履歴に記録するステータスを `"completed"` ではなく **`"saturated"`** に変更して保存します。これにより、信頼性の低いデータが正常データとして埋もれてしまうのを防ぎます。

### 2.10 測定データの蓄積と ROI 解析 API

自動測定中のデータバッファリングと、カメラの解析結果を取得するための API 群です。

#### 2.10.1 AutoMeasurementState (メモリ内バッファリング)
測定中のデータを高速に取得・グラフ化するため、バックエンドのメモリ上にデータ蓄積用のクラス `AutoMeasurementState` を保持しています。

*   **トリガーベース設計**: 測定精度の担保（撮影画像と数値の完全な一致）のため、Snapshot 指示があった瞬間の「確定データ」のみを蓄積する設計を採用しています。
*   **画像パスの記録**: 各測定点に対し、輝度データだけでなく保存された **TIFF画像のファイルパス** もセットで記録します。これにより、グラフ上の特定の点をクリックして対応する画像を即座にプレビューするような高度なUI機能の実現が可能になります。
*   **メモリ保護**: 最大 10,000 点のリングバッファ構造をとり、長時間稼働によるメモリ圧迫を防ぎます。

### 2.11 精密自動測定シーケンス (Step & Shoot)

Sweep測定（連続回転）とは対照的に、特定の角度で「止まって、撮る」を繰り返す精密測定機能です。

#### エンドポイント一覧

| メソッド | パス | 役割 | 備考 |
| :--- | :--- | :--- | :--- |
| `POST` | `/measurement/auto/run` | 自動測定開始 | 開始・終了・ステップ角、保存ディレクトリを指定。 |
| `GET` | `/measurement/auto/progress` | 進捗状況の取得 | パーセンテージ、現在の角度、メッセージを返却。 |
| `POST` | `/measurement/auto/cancel` | 測定のキャンセル | 実行中のループを安全に中断します。 |

#### アーキテクチャの要点
1. **バックグラウンド実行 (`_run_auto_measurement`):**
   - APIリクエストとは別のスレッドで実行され、ステージ移動の完了待ちや物理的な振動減衰待ち（0.3秒）などのブロッキング処理を安全に行います。
   - **角度リストの安全な生成**: 開始・終了・ステップ角度から、測定対象となる全角度のリスト (`angles`) を事前に生成します。浮動小数点誤差（例: 45.00000000000001）による無限ループや予期せぬステップの欠落を防ぐため、微小なマージン（`1e-6`）を用いた安全な比較ロジックと丸め処理を採用しています。
   - **進捗計算の正確性**: 生成されたリストの総数 (`num_steps`) に基づいて進捗パーセンテージを算出することで、UIのプログレスバーと実機の動作を完全に同期させています。
   - **移動開始直後の is_busy 強制セットによる競合回避**: 
     シリアルI/O負荷低減のため、`stage_monitor_loop` はステージ静止時のポーリング間隔を 1.0秒に緩和しています。そのため、ステージ移動コマンド（`move_absolute`, `move_relative`）を送った直後に、プログラム側で即座に `app_state.is_busy = True` を明示的にセットするようにしました。これにより、監視ループが起床してビジー状態を検知するまでのタイムラグによって「移動が開始される前に完了判定ループをすり抜け、静止を待たずに次の撮影や移動コマンドが重複送信されてしまう」という深刻なタイムラグ競合（実機での NG エラーの主原因）を完全に防いでいます。
2. **排他制御 (`auto_operation_lock`):**
   - 実行中の測定状態を保護し、多重起動や不整合を防ぎます。
3. **堅牢なクリーンアップとデッドロックの防止 (`finally`):**
   - 測定中にディスク容量不足（I/Oエラー）や物理シリアルポートの切断といった予期せぬ例外が発生しても、システム全体が「測定中」のままフリーズして操作不能（デッドロック）になるのを防ぐため、スレッドの `finally` ブロックの先頭（最優先）で測定ロックフラグ（`app_state.is_measuring`）を確実に解除します。これにより、後続のファイル保存や自動原点復帰の物理制御でさらにエラーが起きても、UIは安全に操作可能状態に復旧できます。
4. **データ保護とトレーサビリティ:**
   - **中断時の画像保持**: ユーザーが測定をキャンセル（中止）した場合でも、それまでに撮影された個別画像は削除せず保持します。これにより、不完全なデータであっても再解析やトラブルシューティングに使用できます。
   - **詳細な履歴記録**: 測定終了時に、親フォルダの `settings.json`（目次）と枝番フォルダ内の `measurement_details.json` の両方に、最終ステータス（succeeded/failed/cancelled）と詳細メッセージ、および開始・終了時刻を記録します。
5. **自動原点復帰 (Auto Homing) とビジー待機保護:**
   - 測定が完了（または中断）した際、バックグラウンドスレッドの最後で `stage.home()` を呼び出します。これにより、物理センサーを用いた正確な機械原点への復帰と脱調のリセットが行われ、ケーブルのねじれ防止と次測定へのスループット向上を両立させます。
   - **Homing 前の静止待機保護**: 測定エラーが発生して異常終了した直後など、ステージがまだ物理的に移動中である場合に間髪入れずに Homing コマンドを送ると、コントローラが `NG`（ビジーエラー）を返して原点復帰自体が失敗します。これを防ぐため、`stage.home()` を呼ぶ前に `app_state.is_busy` を確認し、ビジー状態であれば最大15秒間、ステージの完全な静止を待ってからコマンドを送る保護処理を導入しました。
6. **後処理（ポストプロセス）の自動化:**
   - ループ終了後（キャンセル時を除く）、同じスレッド内で「マルチページTIFFへの統合」を実行し、完了後に一時的な個別画像をクリーンアップします。

#### 2.10.2 ROI・測定データ API

| メソッド | パス | 役割 | 備考 |
| :--- | :--- | :--- | :--- |
| `POST` | `/camera/rois` | ROI 設定の更新 | 解析対象の座標とサイズを一括で設定します。 |
| `GET` | `/camera/rois` | ROI 設定の取得 | 現在設定されているROIの座標とサイズの一覧を取得します（UIとの同期用）。 |

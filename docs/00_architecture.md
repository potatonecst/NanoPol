...
1.  **フロントエンド (クライアント):**
    ...
    *   **Zustandによる状態管理の局所一元化**: 保存先フォルダパス（`outputDirectory`）や登録プロファイル一覧（`outputPresets`）、選択中ID（`activePresetId`）などをストアで一元管理することで、設定画面での編集保存と、接続画面での切り替え操作のリアルタイム同期をシームレスに行っています。

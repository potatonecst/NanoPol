...
`pyueye` の import が失敗した場合、以下の診断情報を保持します。

*   `pyueye_import_error`: 例外メッセージ全文（例: `could not find any library for ueye_api (DLL_PATH: unset)`）。
*   `pyueye_module_file`: import成功時に読み込まれたモジュール実体ファイル位置。

これらは `GET /camera/diagnostics` で取得でき、`pyueye` 本体不在と uEye DLL 解決失敗の切り分けに使います。

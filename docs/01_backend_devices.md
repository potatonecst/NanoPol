...
*   **【通信ロスト時の自動切断 (ゾンビ化防止)】**: 監視ポーリング（`try_get_status`）中やコマンド送信中に Windows の `PermissionError(13)` や `SerialException`、`OSError` などの物理的な通信ロストを検知した場合、システムは即座に `_mark_disconnected()` を呼び出してシリアル接続を自らクローズし、`is_connected = False` へ遷移させます。これにより、エラーログが毎秒何十回も吐き出され続ける「ゾンビ接続状態」を防ぎ、フロントエンド側へ即座に切断状態を伝播させます。
*   **【自動再接続による自動復旧 (オートリカバリー)】**: 通信エラーによって自動切断された際、ステージコントローラは直近の接続ポート（`last_connected_port`）を記憶し、`is_healing = True` とします。その後、バックグラウンドの監視ループが5.0秒おきに `connect()` を自動再試行し、OSがポートを再認識したタイミングで安全に自動復帰（元の加減速速度の再適用を含む）を完了させます。復帰完了時には専用の復旧ログが出力されます。
*   **【デコードエラーの耐性化】**: シリアル回線への電気的ノイズやUSBバスハング等で文字化けバイト（`0xfc` 等の非ASCII文字）が混入した場合でも、デコード処理で `UnicodeDecodeError` を発生させず `errors="replace"` を用いて安全に代替文字に置換し、パース例外（`ValueError`等）として安全に処理（破棄）します。これにより、ノイズによるシステム全体のクラッシュを排除します。
...
- **自動再接続による自己修復 (Self-Healing) とその改善**:
  *   **一時的タイムアウトのフレームドロップ化（しきい値の緩和）**:
      USBハブ共有環境などで発生する瞬間的なパケット詰まりや電圧ドロップによるタイムアウト例外 (`uc480TimeoutError`) に対し、以前の「3回連続失敗で強制リセット」という判定は過敏すぎ、一時的な混雑で自らカメラを切断・再起動して自滅（無限再起動ループによるフリーズ）を引き起こしていました。このため、しきい値を **`10回連続（約10秒間）`** に緩和しました。10回未満の単発タイムアウトは「ただの1コマのフレームドロップ」として安全にスキップ（`continue`）し、カメラ接続を維持したまま次のコマを待ち受けます。
  *   **再接続・設定適用プロセスの直列化（競合の完全排除）**:
      自己修復スレッド内での再接続直後に、画像取得スレッドが即座に起動してしまうと、その後の設定の再適用（`set_exposure`/`set_gain`）と画像取得（`snap()`）が同じ `_camera_lock` を奪い合い、再起動したその瞬間に再びタイムアウトを誘発するバグがありました。
      これを防ぐため、接続時は `self.connect(start_loop=False)` として画像取得スレッドの起動を保留し、露出とゲインの設定書き込みを**100%完了させた「後」**に、明示的に `self.start_capture_loop()` を呼び出して撮影を開始する直列化設計を適用しました。これにより、再起動時のクラッシュ競合を完全に排除しています。
  *   **自己修復状態フラグの露出**:
      自己修復プロセスが実行中であることを外部から識別可能にするため、メンバ変数 `self.is_healing: bool` および現在の試行回数 `self.reconnect_attempt: int` を追加しました。これらの値は `/health` API を介してフロントエンドに公開され、修復中のUI警告表示と重複接続の防止に利用されます。
...
#### `connect(camera_id: int = 0, start_loop: bool = True) -> bool`
*   **引数:**
    *   `camera_id`: 接続するカメラのデバイスID。
    *   `start_loop`: 接続成功時に直ちに画像取得スレッドを起動するかどうか。自己修復時の競合防止には `False` を指定します。
*   **処理内容:**
...
*   **詳細:** スレッドが既に走行中の場合は二重起動を防ぎ、自己修復処理時に「接続 ➔ 設定再適用完了 ➔ 撮影開始」という安全な直列化シーケンスを担保するために呼び出されます。

...
### 4.2. パフォーマンスと堅牢性
*   **高速性:** NumPy のスライス機能とベクトル演算を利用。パッチサイズが小さい（例: 21x21）場合、計算時間は数マイクロ秒であり、30fps以上のリアルタイム処理においても CPU 負荷は無視できるレベルです。
...
#### `take_snapshot() -> Optional[dict]`
従来の「ファイルパスを返すだけ」の機能から、**「撮影・解析・保存のパッケージ」**を返す機能に進化しました。
*   **戻り値:**
    *   `angle`: 撮影時のステージ角度。
    *   `roi_stats`: この画像から直接計算された最新の ROI 解析結果。
    *   `filepath`: 保存先パス（自動保存時）または `"PENDING"`（ダイアログ待ち時）。
    *   `timestamp`: 撮影完了時刻。

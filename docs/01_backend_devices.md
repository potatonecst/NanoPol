...
#### 排他制御と直列化 (`_io_lock` と `is_priority_locked`, `request_stage_wakeup`)
...
また、静止時（1.0秒ポーリング間隔）のスリープ中に移動命令が発行された際、監視タスクの目覚めを待つタイムラグ（表示のカクつき）を排除するため、`request_stage_wakeup()` を介して非同期割込イベント (`stage_wakeup_event = asyncio.Event()`) を発行します。これにより、監視タスクは0秒でスリープを途中解除して飛び起き、即座に実機から角度の読み取りを開始して 0.1秒周期の高速モードへ移行します。
